#include <UI/CtrlRaceBase/Speedometer2.hpp>
#include <Settings/Settings.hpp>

namespace Pulsar {
namespace UI {
u32 CtrlRaceSpeedo2::Count() {
...
void CtrlRaceSpeedo2::Create(Page& page, u32 index, u32 count) {
    u8 speedoType = (count == 3) ? 4 : count;
    for(int i = 0; i < count; ++i) {
        CtrlRaceSpeedo2* som = new(CtrlRaceSpeedo2);
        page.AddControl(index + i, *som, 0);
        char variant[0x20];
        int pos = i;
        if(i == 0 && Settings::Mgr::GetSettingValue(Settings::SETTINGSTYPE_RACE, SETTINGRACE_RADIO_SOM) == RACESETTING_SOM_OG) pos = 1;
        snprintf(variant, 0x20, "Speedo2_%1d_%1d", speedoType, pos);
        som->Load(variant, i);
    }
}
static CustomCtrlBuilder SOM(CtrlRaceSpeedo2::Count, CtrlRaceSpeedo2::Create);

void CtrlRaceSpeedo2::Load(const char* variant, u8 id) {
    this->hudSlotId = id;
...
void CtrlRaceSpeedo2::Animate(const SpeedArg* args) {
    for(int i = 0; i < 7; ++i) {
